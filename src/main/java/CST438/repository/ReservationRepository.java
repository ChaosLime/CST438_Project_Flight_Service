...
public interface ReservationRepository extends JpaRepository<Reservation, Long> {
  ...
  @Query(value = "UPDATE `reservations` SET `is_cancelled` = 1 WHERE (`book_id` = ?1)",
      nativeQuery = true)
  void cancelBooking(Long id);

  @Query(value = "SELECT * FROM reservations WHERE user_email = ?1 AND is_cancelled = '0'",
      nativeQuery = true)
  List<Reservation> findActiveBookings(String email);

  @Query(value = "SELECT * FROM reservations WHERE user_email = ?1 AND is_cancelled = '1'",
      nativeQuery = true)
